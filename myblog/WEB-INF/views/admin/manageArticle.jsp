...
<!-- Default box -->
      <div class="box">
        <div class="box-header with-border">
          ...
          <form action="/admin/managerArticleServlet" class="form-inline pull-left" >
            <input type="text" class="form-control" name="keys" id="keys" value="${keys}"/>
            <button class="btn btn-primary"><i class="fa fa-search"></i></button>
          </form>
          
          <a href="/admin/addArticleServlet" class="btn btn-success pull-right">写文章</a>
        </div>
          ...
          <table class="table" id="cust_table">
            ...
            <tbody>
              ...
              <c:forEach items="${page.pageItems}" var="article">
               <tr>
                  <td><a href="/user/articleDetailServlet?articleId=${article.id}">${article.title}</a></td>
                  <td><a href="/admin/managerArticleServlet?nodeId=${article.nodeid}">${article.nodeName}</a></td>
                  <td>
                  ...
                  <td>
                    <a class="del" href="javascript:;" rel="${article.id}">删除</a> 
                    <a href="/admin/changeArticleServlet?articleId=${article.id}">修改</a> 
                  </td>
                </tr>
...
<script>
    $(function(){
        ...
        $("#pagination").twbsPagination({
         ...
         visiblePages:3,
         href:"/admin/managerArticleServlet?p={{number}}&nodeId=${param.nodeId}&labelId=${param.labelId}&keys=" + encodeURIComponent("${param.keys}"),
         first: "首页",
         prev: "上一页",
         next:"下一页",
         last:"末页"
